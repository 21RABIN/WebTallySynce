...
class PayrollApiControllerTest {
    ...
    void employeesRouteForwardsToConnectorEmployeesPath() {
        ...
        TallyPostWriteRefreshService refreshService = mock(TallyPostWriteRefreshService.class);
        HttpServletRequest request = mock(HttpServletRequest.class);
        when(gatewayService.forward(HttpMethod.GET, "/employees", request, null))
                .thenReturn(ResponseEntity.ok("{}"));

        PayrollApiController controller = new PayrollApiController(gatewayService, refreshService);
