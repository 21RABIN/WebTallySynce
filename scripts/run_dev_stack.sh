...
start_service \
  ...
  "if [[ -x \"$ROOT_DIR/scripts/mariadb-local.sh\" ]]; then \"$ROOT_DIR/scripts/mariadb-local.sh\" start >/dev/null; fi; if [[ -f \"$LOCAL_DB_ENV_FILE\" ]]; then source \"$LOCAL_DB_ENV_FILE\"; fi; exec mvn spring-boot:run"
