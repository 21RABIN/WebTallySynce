...
export class DashboardHomeComponent implements OnInit, OnDestroy {
  ...
  cacheSources: string[] = [];
  selectedCompany = '';
  readonly defaultRangeStart = '20260401';
  readonly defaultRangeEnd = this.formatDateValue(new Date());
  rangeStart = this.defaultRangeStart;
  rangeEnd = this.defaultRangeEnd;
  rangeStartInput = this.formatInputDate(this.defaultRangeStart);
  rangeEndInput = this.formatInputDate(this.defaultRangeEnd);
  ...
  applyDateRange(): void {
    const nextStart = this.compactDateValue(this.rangeStartInput);
    const nextEnd = this.compactDateValue(this.rangeEndInput);

    if (!nextStart || !nextEnd) {
      this.dateRangeError = 'Choose both From Date and To Date to filter the dashboard.';
      return;
    }
  ...
  resetDateRange(): void {
    this.dateRangeError = '';
    this.rangeStart = this.defaultRangeStart;
    this.rangeEnd = this.defaultRangeEnd;
    this.rangeStartInput = this.formatInputDate(this.defaultRangeStart);
    this.rangeEndInput = this.formatInputDate(this.defaultRangeEnd);
    this.load();
  }
  ...
  private compactDateValue(value: string): string {
    const digits = (value || '').replace(/\D/g, '');
    return digits.length === 8 ? digits : '';
  }

  private formatInputDate(value: string): string {
    const digits = this.compactDateValue(value);
    if (!digits) {
      return '';
    }
