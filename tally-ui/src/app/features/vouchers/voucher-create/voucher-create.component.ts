...
export class VoucherCreateComponent implements OnInit, OnDestroy {
  ...
  readonly form = this.formBuilder.group({
    ...
    voucherNumber: ['101', Validators.required],
  ...
  applyEndpointPreset(endpointKey: string | null): void {
    ...
    const voucherNumber = endpoint.requestKind === 'compliance' ? 'SI-1001' : '101';
  ...
  private buildImportXmlEnvelope(
    ...
    const voucherNumber = this.escapeXml(rawValue.voucherNumber || '101');
...
<AMOUNT>${negativeAmount}</AMOUNT>
  ...
  private resolveError(error: HttpErrorResponse): string {
    ...
    return 'Unable to submit the voucher request.';
  }
